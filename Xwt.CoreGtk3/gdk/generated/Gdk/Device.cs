...
#region Autogenerated code
	public partial class Device : GLib.Object {
		...
		private bool InternalGetHistory (Gdk.Window window, uint start, uint stop, Gdk.TimeCoord events, out int n_events)
		{
			...
			events = Gdk.TimeCoord.New (native_events);
		...
		[UnmanagedFunctionPointer (CallingConvention.Cdecl)]
		delegate void WarpNativeDelegate (IntPtr inst, IntPtr screen, int x, int y);

		static void Warp_cb (IntPtr inst, IntPtr screen, int x, int y)
		{
			try {
				Device __obj = GLib.Object.GetObject (inst, false) as Device;
		...
		protected virtual void OnWarp (Gdk.Screen screen, int x, int y)
		{
			InternalWarp (screen, x, y);
		}

		private void InternalWarp (Gdk.Screen screen, int x, int y)
		{
		...
		[UnmanagedFunctionPointer (CallingConvention.Cdecl)]
		delegate bool QueryStateNativeDelegate (IntPtr inst, IntPtr window, IntPtr root_window, IntPtr child_window, out int root_x, out int root_y, out int win_x, out int win_y, out int mask);

		static bool QueryState_cb (IntPtr inst, IntPtr window, IntPtr root_window, IntPtr child_window, out int root_x, out int root_y, out int win_x, out int win_y, out int mask)
		{
			try {
				Device __obj = GLib.Object.GetObject (inst, false) as Device;
				bool __result;
				Gdk.ModifierType mymask;
				__result = __obj.OnQueryState (GLib.Object.GetObject(window) as Gdk.Window, GLib.Object.GetObject(root_window) as Gdk.Window, GLib.Object.GetObject(child_window) as Gdk.Window, out root_x, out root_y, out win_x, out win_y, out mymask);
				mask = (int) mymask;
				return __result;
			} catch (Exception e) {
				GLib.ExceptionManager.RaiseUnhandledException (e, true);
		...
		protected virtual bool OnQueryState (Gdk.Window window, Gdk.Window root_window, Gdk.Window child_window, out int root_x, out int root_y, out int win_x, out int win_y, out Gdk.ModifierType mask)
		{
			return InternalQueryState (window, root_window, child_window, out root_x, out root_y, out win_x, out win_y, out mask);
		}

		private bool InternalQueryState (Gdk.Window window, Gdk.Window root_window, Gdk.Window child_window, out int root_x, out int root_y, out int win_x, out int win_y, out Gdk.ModifierType mask)
		{
			QueryStateNativeDelegate unmanaged = GetClassStruct (this.LookupGType ().GetThresholdType (), true).QueryState;
			if (unmanaged == null) throw new InvalidOperationException ("No base method to invoke");

			int native_mask;
			bool __result = unmanaged (this.Handle, window == null ? IntPtr.Zero : window.Handle, root_window == null ? IntPtr.Zero : root_window.Handle, child_window == null ? IntPtr.Zero : child_window.Handle, out root_x, out root_y, out win_x, out win_y, out native_mask);
			mask = (Gdk.ModifierType) native_mask;
			return __result;
		}
		...
		[UnmanagedFunctionPointer (CallingConvention.Cdecl)]
		delegate IntPtr WindowAtPositionNativeDelegate (IntPtr inst, out int win_x, out int win_y, out int mask, bool get_toplevel);

		static IntPtr WindowAtPosition_cb (IntPtr inst, out int win_x, out int win_y, out int mask, bool get_toplevel)
		{
			try {
				Device __obj = GLib.Object.GetObject (inst, false) as Device;
				Gdk.Window __result;
		...
		protected virtual Gdk.Window OnWindowAtPosition (out int win_x, out int win_y, out Gdk.ModifierType mask, bool get_toplevel)
		{
			return InternalWindowAtPosition (out win_x, out win_y, out mask, get_toplevel);
		}

		private Gdk.Window InternalWindowAtPosition (out int win_x, out int win_y, out Gdk.ModifierType mask, bool get_toplevel)
		{
